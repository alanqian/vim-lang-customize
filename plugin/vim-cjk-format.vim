...
"
" "format_allow_over_textwidth"
"
" The width that can over 'textwidth'. This variable is used for taboo rule.
"
if !exists("g:format_allow_over_textwidth")
  let g:format_allow_over_textwidth = 2
endif
...
function! s:FormatLine(second_indent)
  ...
  while s:GetWidth() > virtcol(".")
    ...
    let force_fold = 0
    let do_insert = 0
    let max_width = virtcol(".") + g:format_allow_over_textwidth
    ...
    if !finish_format && !force_fold && g:format_follow_taboo_rule
      ...
      let next_ch = s:GetCharUnderCursor()
      normal! h
      if s:TabooRuleMatch(g:format_forbid_begin, next_ch)
        normal! l
        while s:TabooRuleMatch(g:format_forbid_begin, next_ch)
      ...
      if virtcol(".") > max_width
        let finish_format = 0
        while s:TabooRuleMatch(g:format_forbid_begin, ch)
      ...
      let ch = s:GetCharUnderCursor()
      if s:TabooRuleMatch(g:format_forbid_end, ch) && !force_fold
        let do_insert = 0
        while s:TabooRuleMatch(g:format_forbid_end, ch)
            \&& virtcol(".") -1 > leader_width
          normal! h
          let ch = s:GetCharUnderCursor()
        endwhile
...
"
" let l:uni_char = strcharpart(getline('.')[col('.') - 1:], 0, 1) " can hold 中文
